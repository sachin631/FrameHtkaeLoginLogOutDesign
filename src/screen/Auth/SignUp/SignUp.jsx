...
export default function SingUp() {
  ...
  const [passwordErr,setErrorPassword]=useState(false)
  ...
  async function SignUp() {
    ...
    console.warn(item)
    let result = await fetch("http://ec2-52-66-165-70.ap-south-1.compute.amazonaws.com:3000/user/register", {
      method: 'POST',
  ...
  return (
    <div className="min-h-screen min-w-screen flex justify-center items-center bg-white">
      <div className='sign-up-form-input-shadow w-[100vw] mx-auto bg-white grid grid-cols-12 rouned-xl overflow-hidden'>
        {/* remove container grid-cols-10 rounded-md*/}
        ...
        <div className="col-span-6 flex justify-center  items-center">
          <div className="p-8 w-full max-w-[523px]">
            ...
            <p className='text-[18px] leading-[21.78px]'>Already have an account? <NavLink to="/Login"><span className='text-primary-400'>Log in</span></NavLink></p>
            <form action="" method="POST" className='w-full mt-[48px] space-y-[30px]'>
              <div>
                ...
                <div className="p-[16px]  mt-[12px] flex items-center border border-[#F2F2EC] rounded-full sign-up-form-input-shadow w-full">
                  ...
                  {/*------------------name inputbar-------------------------------  */}
                  <input type="text" value={name} onChange={(event) => { userhandler(event) }} placeholder='Your Name' className='text-[16px]  text-[#4C4C4A] w-full  outline-none leading-[19.36px] ml-[17.16px] placeholder:text-[#B2B2AE]' required />

                </div>
                {nameErr ? <span className='text-red-500'>! invalid name (must be larger then 2 letter) </span> : ""}
              </div>
              <div>
                ...
                <div className="p-[16px]  mt-[12px] flex items-center border border-[#F2F2EC] rounded-full sign-up-form-input-shadow w-full">
                  ...
                  {/*--------------- emial id input bar-------------- */}
                  <input type="email" value={email} onChange={(event) => { emailhandler(event) }} placeholder='Enter your email id' className=' text-[#4C4C4A] w-full text-[16px] outline-none leading-[19.36px] ml-[17.16px] placeholder:text-[#B2B2AE]' required />
                </div>
                {emailErr?<h1 className='text-red-500'>!invalid (email must be larger then 4 letter and "@" "." must be at right place)</h1>:""}
              </div>
              <div>
                ...
                <div className="p-[16px]  mt-[12px] flex items-center border border-[#F2F2EC] rounded-full sign-up-form-input-shadow w-full">
                  ...
                  {/*------------------ password input bar ----------------------*/}
                  <input type="password" value={password} onChange={(event) => {passwordhandler(event)} } placeholder='Password' className='text-[16px] w-full text-[#4C4C4A]  outline-none leading-[19.36px] ml-[17.16px] placeholder:text-[#B2B2AE]' required />
                
                </div>
                {passwordErr?<h1 className='text-[#FF2E00] leading-[19px] font-[16px] font-[400] font-[Inter] flex justify-start items-center mt-[10px] gap-[12px]'><img src="../../images/error.png" alt="icon"/>Wrong Password</h1>:""}
              </div>

              {/* Terms & Conditions */}
              <div className="form-check form-switch">
                <input className="form-check-input appearance-none w-9 -ml-10 rounded-full float-left h-5 align-top checked:bg-primary-400 bg-white bg-no-repeat bg-contain bg-gray-300 focus:outline-none cursor-pointer shadow-sm" type="checkbox" role="switch" id="flexSwitchCheckDefault" required />
                <label className="form-check-label inline-block text-[#B2B2AE]  text-[16px] leading-[19.8px] ml-[10px]" htmlFor="flexSwitchCheckDefault"><p>I agree to <span className='text-primary-400'>Platform Terms</span> and services and <span className='text-primary-400'>Privacy Polocy</span></p></label>
              </div>

              {/* Create Account */}
              <div className="mt-[42px]">
                <button onClick={SignUp} className='bg-primary-400 shadow-lg shadow-primary-400/50 text-white text-[18px] p-[13px] leading-[21.78px] rounded-full w-full font hover:bg-[#FA5916]'>Create Account</button>
              </div>

              {/* Or Divider */}
              <div className="relative flex py-5 items-center">
                <div className="flex-grow border-t border-[#B2B2AE]"></div>
                <span className="flex-shrink mx-[13px] text-[#B2B2AE]">Or</span>
                <div className="flex-grow border-t border-[#B2B2AE]"></div>
              </div>
